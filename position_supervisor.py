...
class PositionSupervisor:
    ...
    def notify_open_success(self, signal: str, symbol: str, qty: float, entry_price: float):
        with self.lock:
            try:
                ...
                # 调用 binance_client 计算 TP 并更新 position_manager
                ...
                # 初始化时 stop_loss 为 None（由 tp_monitor 在 TP1 后设置）
                self.position_manager.update_position(
                    ...
                    stop_loss=None   # TP1 触发后由 tp_monitor 自动设置
                )

                self.last_signal = signal
                self.consecutive_failure_count = 0
                logging.info(f"[监督层] {direction} 开仓成功，持仓已初始化（stop_loss 待 TP1 后设置）")
    ...
    def notify_manual_intervention(self, change_type: str, symbol: str, side: str,
        ...
        with self.lock:
            try:
                logging.info(f"[监督层] 收到人工{change_type}通知")
