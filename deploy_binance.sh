...
DEPLOY_SCRIPT_VERSION="v16.5-set-plus-m"
...
force_cleanup() {
    ...
    while [ "$round" -le "$MAX_CLEANUP_ROUNDS" ]; do
        ...
        if ! port_in_use; then
            ...
            # 必须清掉按品种锁，否则多 worker / PID 复用会跳过闪电接管钉钉
            rm -f "${DIR}/logs/.recover_singleton.lock" \
                  "${DIR}/logs/.recover_singleton_"*.lock 2>/dev/null || true
            return 0
        fi
        round=$((round + 1))
    done
