...
# 2026-08-27新增：sqrt(TSLA breath_profiles.py中位数回调3.40/ETH当前中位数
# 回调2.70)≈1.122倍放宽ETH三档基线（210.5天842根原生6h K线、120个回调样本）。
_DEFAULT_TSLA_TIERS: List[Dict[str, float]] = [
    {"step_trigger_atr": 1.12, "step_advance_atr": 0.56,
     "breath_tp12": 1.50, "breath_tp23": 2.00, "min_mult": 2.5, "max_mult": 3.5},
    {"step_trigger_atr": 1.35, "step_advance_atr": 0.67,
     "breath_tp12": 2.00, "breath_tp23": 2.80, "min_mult": 3.0, "max_mult": 4.5},
...
# 2026-09-08新增：宝贝要求"增加合约STXXUSDT.P，75分钟周期"。核实币安
# TRADIFI_PERPETUAL(underlyingType=EQUITY)、状态TRADING，跟GS/MU/LITE/
# TSLA/META/DELL/GEV同族(stepSize=0.01/minQty=0.01/tickSize=0.01)。
# sqrt(STXX breath_profiles.py中位数回调2.46/ETH当前中位数回调2.37)≈
# 1.019倍微放宽ETH三档基线（75分钟不是原生间隔，能被15整除，用15m原始
# K线合成；86.5天1661根合成75分钟K线、234个回调样本，样本量充足）。
_DEFAULT_STXX_TIERS: List[Dict[str, float]] = [
    {"step_trigger_atr": 1.02, "step_advance_atr": 0.51,
     "breath_tp12": 1.50, "breath_tp23": 2.00, "min_mult": 2.5, "max_mult": 3.5},
    {"step_trigger_atr": 1.22, "step_advance_atr": 0.61,
     "breath_tp12": 2.00, "breath_tp23": 2.80, "min_mult": 3.0, "max_mult": 4.5},
...
MAX_REENTRIES = int(_CFG.get("max_reentries") or 2)  # config/reentry_tiers.json同步为2
...
# STXX(75min)：2×75=150min，正好落在ETH 2×90m=180min一带的~150-200min
...
REENTRY_TSLA: Dict[str, Any] = {
    "name": "TSLA",
    "tv_tf": "6h",
    "tv_tf_sec": _TSLA_TF_SEC,
    "enabled": True,
    "arm_sl_atr": ARM_SL_ATR,
    "fee_cover_pct": FEE_COVER_PCT,
    "arm_mode": ARM_MODE,
    # 2026-08-27：TSLA ATR%=1.35%，沿用跟其它TradFi品种一致的1%惯例。
    "radar_gate_return_pct": 0.01,
    "tiers": TSLA_TIERS,
    "reentry_zone_atr": _TSLA_ZONE,
    "reentry_window_bars": _TSLA_WINDOW_BARS,
    "limit_discount": LIMIT_DISCOUNT,
    "limit_ttl_sec": LIMIT_TTL_SEC,
    "max_reentries": MAX_REENTRIES,
    "max_unfilled_refreshes": MAX_UNFILLED_REFRESHES,
...
REENTRY_STXX: Dict[str, Any] = {
    "name": "STXX",
    "tv_tf": "75m",
    "tv_tf_sec": _STXX_TF_SEC,
    "enabled": True,
    "arm_sl_atr": ARM_SL_ATR,
    "fee_cover_pct": FEE_COVER_PCT,
    "arm_mode": ARM_MODE,
    # 2026-09-08：STXX ATR%=0.64%，沿用跟其它TradFi品种一致的1%惯例，
    # 不单独客制化收紧。
    "radar_gate_return_pct": 0.01,
    "tiers": STXX_TIERS,
    "reentry_zone_atr": _STXX_ZONE,
    "reentry_window_bars": _STXX_WINDOW_BARS,
    "limit_discount": LIMIT_DISCOUNT,
    "limit_ttl_sec": LIMIT_TTL_SEC,
    "max_reentries": MAX_REENTRIES,
    "max_unfilled_refreshes": MAX_UNFILLED_REFRESHES,
...
RADAR_GATE_ATR_MULT_BY_TIER = {0: 1.5, 1: 1.8, 2: 2.5}
...
# 同一天再追加：weak/mid此前共用同一个1.5倍值，没有真正区分开——用户
# 反馈"弱/中/强都该有自己的激活时机"，mid档单独上调到1.8倍，给中等
# 置信度的趋势也留一点点额外空间；weak维持1.5不变，弱趋势更容易是假
# 突破，继续保留最早的保护。注意：mid档目前仍保留radar_gate_return_pct
# 这条腿(只有strong档关闭了)，多数品种这条腿(~1-1.5%)还是会比1.8×ATR
# 更紧，所以这次调整对mid档的实际效果有限——如果观察下来还是不够，
# 需要再讨论是否也给mid档放宽/关闭这条腿。
...
def radar_gate_price_from_tps(
    ...
    **kwargs,
    ...
    # 这条线当初是为了保护弱趋势档"浮盈在ATR线触发前被打回去"，对弱档
    # 仍然有效、维持不变；强趋势档单独关闭。
    # 同一天再追加：中档(tier=1)ATR腿从1.5拉到1.8之后核算发现——中档
    # 收益率线(以ZEC 1%为例)只有8.11，就算放宽1.5倍也才12.17，跟1.8×ATR
    # 算出的62.15、TP1线的51.42差距太大，min()还是选中收益率线，等于
    # ATR腿的放宽白做。索性中档也一并关闭这条腿，回归纯TP1/ATR双触发——
    # 这样弱/中/强三档才是真正递进(弱档保留最早保护，中/强档给足空间、
    # 靠各自不同的ATR倍数区分)，不是"强档单独特殊、弱中还是同一套"。
    rp = float(return_pct or 0) if tier_i == 0 else 0.0
