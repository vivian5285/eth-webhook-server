...
## 当前有效架构（2026-07 · v16.9.1-tv-ready）
...
├── 永久硬止损 |TV−SL|×1.15 @ 成交价 + 独立雷达呼吸止损（只读 TV webhook atr）
...
| 规则 | 实现 |
|------|------|
| 缓存窗口 **固定 1.0s** | `SAME_BAR_SETTLE_SEC` = 1.0 |
| 同bar有平仓 → **一律先平后开** | 平仓一次 + 最新开仓 |
| 60s 去重 | `action+symbol+price` |
| TP 分腿 | **10/20/70**，仅挂 TP1+TP2 限价；TP3(70%) 永不挂单，交雷达 |
