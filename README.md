...
**当前版本：`v13.70.0-close-then-open-sterile`**  
...
# 期望 version 含 v13.70.0-close-then-open-sterile
...
- **时序先平后开**（v13.68/v13.70）：同K线只可能单独 CLOSE，或 CLOSE(seq小)+OPEN(seq大)；永无「先开后平」同时发；CLOSE 后释放开仓幂等
- **无菌空仓再开**（v13.70）：开仓前撤尽 TP123/雷达/宽止损 → 平仓归零 → 再撤 → 扫孤儿 → 验 qty=0+orders=0，再挂新方向 TP123+宽硬止损+雷达待命
...
# 期望: v13.70.0-close-then-open-sterile
...
# version 含 v13.70.0 · tv_strategy 含 v6.9.108
