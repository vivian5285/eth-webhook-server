...
**当前版本：`v13.29.0-flat-purge-tp123`**
...
- **计算**：`硬止损距离 = ATR × sl_m × 档位倍数`（TV `tv_sl` 仅参考，不直接挂单）
- **四档呼吸**（ATR≈16）：R1≈30U · R2≈50U · R3≈70U · R4≈100U
- **执行**：币安 `STOP` 限价单（触发价=VPS止损价，限价±0.5U 缓冲防跳空）
- **雷达合并后**：`effective = max(雷达, vps_sl)`（LONG）/ `min`（SHORT）→ `STOP_MARKET closePosition`
- **优先级**：`CLOSE_STOPLOSS` 市价全平 > VPS 缓冲止损 > 忽略 `UPDATE_SL`
- TP1 前：**始终维护**宽止损呼吸空间

| Regime | sl_m | 档位倍数 | 最终倍数 |
|--------|------|----------|----------|
| 1 | 0.9 | 2.0 | 1.80× |
| 2 | 1.05 | 3.0 | 3.15× |
| 3 | 1.10 | 4.0 | 4.40× |
...
2. 相对 `_trusted_initial_qty` 有对应减仓（约 30%）  
3. 盘口 **无** TP1 限价单残留  
...
**核心理念**：所有档位统一 — **TP1 限价成交后才激活雷达**；TP1 前只有 VPS 宽硬止损，给足呼吸空间。
...
| **v13.29** | **全平/人工平仓后多轮核武撤净 TP123+止损，防止反向 orphan 仓** |
| **v13.28** | **人工首仓接管：禁止 TP1 前误激活雷达；修复 OPEN 日志锚定误判** |
