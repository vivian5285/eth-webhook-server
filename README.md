...
**当前版本：`v13.78.0-open-defense-failclosed`**  
...
# 期望 version 含 v13.78.0-open-defense-failclosed
...
# 期望: v13.78.0-open-defense-failclosed
...
# version 含 v13.78.0 · tv_strategy 含 v6.9.108
...
| 重启后仓位被莫名平掉 / TP1 被削光 | 升级 ≥ **v13.78**；接管禁 force 裁减；真成交才记账；无减仓推离后挂 |
...
> 当前修复版本基准：**`v13.78.0-open-defense-failclosed`**（含 v13.75～v13.77 时序/钉钉/接管铁律）。
...
| 7 | **现价已达 TP1 且真成交 → 禁止当漏挂重挂** | 有减仓证据才跳过；无减仓则推离后仍挂。禁止在 TP1 附近反复重挂把仓吃光。 |
...
- **根因**：补挂/核武路径把「限价不在」当成漏挂，**未先判断现价是否已达该档且确有减仓**；`cancel_all` 连带撤硬止损。  
- **修复**：≥ **v13.77** 现价已达且真成交 → 记账等待；≥ **v13.78** 无减仓证据时 **推离后仍挂**（不假吃、也不在穿价区硬挂）。  
  - 重启 scorch **只撤 TP，保留硬止损**；  
  - 接管跳过 force 档位裁减（禁误减仓/误平）。  
- **正确接管姿势**：  
  1. 读实盘仓位与方向（**禁止无故平仓**）；  
  2. 看 mark 是否已达 TP1 + 是否有减仓 → 决定是否启动雷达；  
  3. 钉钉/日志给出「应挂 TP 价 + 剩余切片数量」；  
  4. 只补 **未真成交且确实缺失** 的档（穿价则推离）。
...
*GEMINI Quant · 双轨智慧雷达 · v13.78.0-open-defense-failclosed*
