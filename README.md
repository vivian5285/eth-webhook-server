...
**当前版本：`v13.64.1-naked-defense-guard`**  
...
# 期望 version 含 v13.64.1-naked-defense-guard
...
```bash
python check_vps_logic.py          # 静态对账（无需 API Key）→ 期望 101 通过
...
├── 雷达待命 → 价触激活线(R1/R2=70% · R3=75% · R4=80%)后交棒保本
├── 13x 总名义硬顶（双品种合计）
...
- **雷达价触激活线启动**：相对 entry→TP1；废除「价+限价成交+减仓」三重强制门槛
...
现价/best 达档位激活线（R1/R2=70% · R3=75% · R4=80%）
...
# 期望: v13.64.1-naked-defense-guard
...
# version 含 v13.64.1 · tv_strategy 含 v6.9.108
...
| 开仓后无 TP、无硬止损 | 升级 ≥ v13.64；查「开仓 TP123 补全」「开仓终检裸仓」；人工先挂 closePosition |
| 只有 TP23 无 TP1 | 查 `伪TP` / `trusted_initial` / 重启 `_ensure_full_defense_stack` |
| 微盈就全平、无雷达钉钉 | 查「交棒延迟」/ SHORT 抬过成本；确认开仓冷却 180s |
...
*GEMINI Quant · 双轨智慧雷达 · v13.64.1-naked-defense-guard*
