...
# 期望 version 含 v13.83.0-tv-defense-iron
...
# version 含 v13.83.0 · tv_strategy 含 v6.9.108
...
> 当前修复版本基准：**`v13.83.0-tv-defense-iron`**（先平后开+TP123+TV硬止损+雷达待命铁律；含 v13.75～v13.82）。
...
### 近期详细更新记录（v13.67 → v13.83）

#### v13.83.0 · `tv-defense-iron`

**主题：TV 到达 → 先平后开 → 挂齐 TP123+TV硬止损 → 雷达候命 → 实盘钉钉；清新老打架**

- **执行顺序铁律**：凡 OPEN 一律无菌先平后开 → 开仓核实 → **挂一次** TP123（档位比例）+ TV `tv_sl` 硬止损 → 雷达阶段0 候命
- **同秒开+平**：动作优先于 seq，终态必须有仓
- **废除**同向「仅刷新 TP」回退路径；`OPEN_SAME_DIR_COOLDOWN=0`；误调改走先平后开
- **三轨不抢份额**：TP=`reduceOnly`；TV硬止损∪雷达=`closePosition` 单槽；雷达按 R1~R4 激活线锁利
- **钉钉文案**全面「TV硬止损」（废除「VPS%宽止损」误导）；开仓核实含激活线/头寸对账
- 修开仓裸仓告警 `vps_target` NameError → `tv_target`

#### v13.82.0 · `tv-risk-sizing`
...
*GEMINI Quant · 双轨智慧雷达 · v13.83.0-tv-defense-iron*
