...
**雷达（马拉松 · v16.8.0）**：激活瞬间**保本起步**（entry±tick±fee）；ADX 三档启动 **弱85%/中80%/强70%** × 1.35×ATR；**取消 TP1/TP2 强制底线**；TP 成交只缩量不改价；阶梯从保本位被动跟随；达线前盘口**仅硬止损**  
...
> **雷达（马拉松 · v16.8.0）**：激活第一步永远是保本（entry±tick±fee），再从保本位阶梯跟随市价；取消 TP1 强制底线；ADX 弱85%/中80%/强70%。  
> **v16.8.0**：马拉松雷达——保本起步 + 取消强制底线 + ADX 三档启动比例翻转（强趋势更早守护）。  
...
# version: v16.6.2-rate-iron · pipeline: {ETHUSDT, XAUUSDT} · trading_paused: false
...
## 六、被动雷达 + 智能再入场（马拉松 · v16.8.0 · 独立于硬止损）

- ETH / XAU 参数只读 `breath_profile` + `reentry_profiles` / `config/reentry_tiers.json`
- **设计哲学**：激活瞬间第一步永远是**保本**，再像马拉松一样跟在市价后面；守护趋势走到 TP3，不提前卡位拦截
- **ADX 三档**：0 弱（<20）/ 1 中（20–30）/ 2 强（>30）；开仓锁定档位 → 启动比例 + 步进/呼吸；硬止损 buffer **恒为 1.15**
- **启动（第一层 · ADX 离散三档）**：
  - 弱 ADX<20 → **85%**；中 20–30 → **80%**；强 >30 → **70%**（强趋势更早守护）
  - `启动价 = entry ± ratio × 1.35 × initial_atr`（首次/重入同一公式）
  - 开仓冻结 ratio + 启动价；**独立于 TP1 是否已成交**；达线前仅硬止损守护
- **激活臂（保本起步）**：止损移到 **entry ± tick ± fee_cover**（约 0.08%），**禁止**跳到 entry±0.5ATR / TP1 底线
- **取消强制底线**：TP1/TP2 成交后**只收缩数量**（→70% / →40%），止损价格不跳变，继续阶梯推进
- **阶梯跟随**：按档位 `step_trigger` / `step_advance` 从保本位被动跟进；浮盈≥3×ATR 切入阶段二连续追踪
...
| 档位 | ADX | 启动% | buffer | ETH step/adv · breath12/23 · trail | XAU step/adv · breath12/23 · trail |
|------|-----|-------|--------|-------------------------------------|-------------------------------------|
| 0 弱 | <20 | **85%** | **1.15** | 0.40/0.25 · 0.8/1.0 · 1.2~1.5 | 0.35/0.20 · 0.7/0.9 · 1.0~1.3 |
| 1 中 | 20–30 | **80%** | **1.15** | 0.50/0.35 · 1.2/1.6 · 1.8~2.5 | 0.40/0.30 · 1.0/1.4 · 1.5~2.0 |
| 2 强 | >30 | **70%** | **1.15** | 0.60/0.40 · 1.5/2.0 · 2.5~3.5 | 0.50/0.35 · 1.3/1.8 · 2.0~2.8 |

再入微赚区：ETH ±0.5×ATR · XAU ±0.3×ATR。配置源：`config/reentry_tiers.json`（schema `v4.0-marathon-breakeven`）。  
...
**一句话**：硬止损是底线（呼吸垫统一 1.15）。马拉松雷达激活瞬间保本起步，再从保本位阶梯跟随；ADX 弱85%/中80%/强70%。取消 TP1 强制底线，让利润跑到 TP3。
...
| 雷达激活 | 首次或重入、**ADX启动比例×1.35ATR**、激活价、初始雷达止损、**趋势档位** |
| 止损移动 | 新止损、浮盈、极值 |
