...
├── **TV 硬止损**（严格按 `tv_sl` 挂 closePosition；废除 VPS% 宽止损）
...
- **硬止损 = TV `tv_sl`**：多空一律按 TV 硬止损挂 `closePosition`；禁止再用开仓价×档位% 的 VPS 宽止损覆盖/拒挂
- **开仓档位以 TV OPEN 为准**（v13.63）：禁止 recover / 粘性日志误锁 R4 覆盖本次开仓 R
- **开仓基数 = 账户总权益**（marginBalance），非可用余额
- **雷达适度追随**（v13.73/v13.74）：按档启动 R1=50%/R2=60%/R3=70%/R4=80%；步进 35/30/25/20%；呼吸 1.0/0.8/0.65/0.5 ATR
...
- **重启禁止无故平仓**：未达激活线且 TP1 未成交 → 只挂 TV 硬止损
...
- **三轨不抢份额**：TP123=`reduceOnly`；雷达保本+TV硬止损=`closePosition` 单槽；各自独立
...
| 开单 sizing | TV `risk_pct`/`leverage`/`qty_ratio` → min(理论,杠杆限)×ratio（**无硬上限**） | 🔴 P0 |
| VPS 硬止损 | 实盘挂 **TV `tv_sl`**（STOP_MARKET closePosition） | 🔴 P0 |
| 开仓裸仓闸 | TV 空 TP → ATR 补全；强制 closePosition；终检钉钉 | 🔴 P0 |
| 13x 名义硬顶 | ETH+XAU 合计 ≤ 权益×13 | 🔴 P0 |
| 雷达适度追随 | R1~R4 激活50/60/70/80% · 步进35/30/25/20% · 呼吸1.0/0.8/0.65/0.5ATR | 🟡 P1 |
...
_enforce_defense_alignment()   ← TP123 比例限价 reduceOnly（纯缺失优先补挂）
  ↓
_maintain_hard_shield()        ← TV 硬止损 closePosition（雷达激活后合并）
  ↓
[若价触激活线]
...
### TV 硬止损（`tv_sl` · closePosition）

- **计算**：实盘硬止损价 = TV **`tv_sl`**（多空严格）；旧「开仓价×档位%」仅作 sizing 距离对照，**不挂盘**
- **TV `tv_sl`**：写入账本并 **挂 closePosition**；`UPDATE_SL` 同步改挂
- **执行（币安）**：Algo / 普通通道 **`STOP_MARKET` + `closePosition=true`**（全平保护，不占 reduceOnly）
- **持仓期**：硬止损随 TV `UPDATE_SL` / 开仓刷新；雷达交棒后可由保本线取代
- **优先级**：`CLOSE_STOPLOSS` 市价全平 > TV 硬止损 / 雷达保本

| Regime | 旧 VPS%对照（不挂盘） | 示例@1800 多头对照价 |
|--------|----------------------|---------------------|
| R1 | **2.78%** | ≈1750.0 |
| R2 | **3.89%** | ≈1730.0 |
| R3 | **5.56%** | ≈1700.0 |
...
**雷达启动**仅看价触激活线；未达激活线却出现保本线 → `_disarm_premature_radar()` / `_enforce_pre_tp1_radar_standby()` 恢复 TV 硬止损。
...
市价成交核实持仓
  ...
  → _refresh_vps_hard_sl(...)                 # 账本写入 TV tv_sl 并挂盘
  → cancel_all 残留挂单（仅一次）
...
**核心理念**：**价格朝 TP1 推进达到档位比例即激活雷达保本**；激活线前只有 TV 硬止损。TP 限价成交仅辅助记账与后续锁利。
...
|------|--------------|------|----------|------|
| R1 | 50% | 35% | 1.0 | 最松，给足空间 |
| R2 | 60% | 30% | 0.8 | 较松 |
| R3 | 70% | 25% | 0.65 | 适中 |
| R4 | 80% | 20% | 0.5 | 稍积极，仍非紧追 |
...
- 价格回调时止损保持不动，**永不回退**
- 雷达止损优先级高于 TV 硬止损（合并为单槽 closePosition）
...
```
开仓
  → TP123（reduceOnly）+ TV 硬止损（closePosition）
  → 雷达待命（阶段0）· mark@1s WS 盯价 · 开仓冷却内禁止交棒

现价达档位激活线（R1=50%…R4=80%）或 TP1 已真实成交（价到+限价消失）
...
| action | 行为 |
|--------|------|
| `LONG` / `SHORT` | **一律先平后开**刷新仓位；空字段本地补全 regime/atr/tp |
| `UPDATE_SL` | **按 TV `tv_sl` 同步盘口硬止损**（多空）；雷达激活时仍可合并保本 |
| `UPDATE_TP` | 更新账本 TP 价并按需重挂限价止盈 |
| `CLOSE` / `CLOSE_PROTECT` / `CLOSE_TP3` / `CLOSE_STOPLOSS` | 撤单 → 全平 → 复位 |

### 反向信号

持多收 `SHORT`（或反之）→ **一律先平后开**。

### 开仓铁律（v13.83）

```
凡带开仓的 TV（同向/反向/空仓）→ 一律无菌先平后开
  → 市价开仓核实
  → 挂一次 TP123（档位比例 reduceOnly）+ TV 硬止损（closePosition）
  → 雷达阶段0 候命（R1=50%…R4=80% 激活线）
  → 实盘核实钉钉一条
同秒开+平 → 动作优先 CLOSE→OPEN（终态必须有仓）
单独平仓 → 清零挂单/账本，干净等待下次 TV
```

> 已废除：同向「仅刷新 TP」、开仓冷却禁先平后开、空仓轻量直开绕过净场。

空仓短时重复同向信号 → 忽略 + 钉钉（无 `bar_index`/`seq` 时指纹去重）。  
...
| 类型 | sizing 规则 |
|------|-------------|
| **OPEN** | `min(理论, 杠杆限制) × qty_ratio`（无硬上限）；直接用 TV `risk_pct`/`leverage`/`qty_ratio` |
| **PYRAMID** | 同一公式，TV `qty_ratio` 约 0.3~0.5 |
| **PROFIT_ADD** | 同上 |

**禁止**旧「档位保证金% × 25x」与「base_qty × ratio」双路径打架。缺 TV `risk_pct`/`leverage` 时拒绝下单。
...
加仓后：撤旧 TP → 按 TV `tv_tp1/2/3`（缺则补全）+ 新总头寸重挂；同步 TV 硬止损 + 雷达（已交棒则推升），钉钉实盘核实。
...
**平仓一眼辨：** 钉钉「🧭 平仓归因」——`📡 雷达保本止损` ≠ `🏆 TP3止盈收网` ≠ `🛡️ TV硬止损`。归因看 `_radar_handoff_done`，不看钉钉是否曾发出。
...
|------|------------------|-------------|---------|------------------|
| R1 | **8%** | **25/35/40** | **2.78%** | **50% · 步进35% · 呼吸1.0 ATR** |
| R2 | **14%** | **20/35/45** | **3.89%** | **60% · 步进30% · 呼吸0.8 ATR** |
| R3 | **20%** | **18/32/50** | **5.56%** | **70% · 步进25% · 呼吸0.65 ATR** |
| R4 | **26%** | **5/20/75** | **8.33%** | **80% · 步进20% · 呼吸0.5 ATR** |
...
# version 含 v13.85.0 · tv_strategy 含 v6.9.108
...
| `tv_sl` | **实盘硬止损挂单价**（多空严格按此挂 closePosition） |
...
| 开仓后无 TP、无硬止损 | 升级 ≥ **v13.81**；查「开仓前防线快照」「绑定开仓防线」「开仓强制挂防线」；人工先挂 closePosition |
...
| 重启后仓位被莫名平掉 / TP1 被削光 | 升级 ≥ **v13.79**；查「开仓价/现价对账」「接管跳过补挂」；已过TP1只挂23+雷达 |
...
> 当前修复版本基准：**`v13.85.0-no-hard-cap`**（无单笔硬上限；含 v13.75～v13.84 铁律）。
...
| 4 | **开仓必须有 TP123 + TV 硬止损** | 缺任一视为裸仓；终检 `_force_hang_open_defenses` 闭环补挂；钉钉告警。雷达开仓后仅待命，不替代硬止损。**只挂一次、挂齐**，禁止秒撤秒挂 thrash。 |
| 5 | **硬止损先挂核实，再撤孤儿** | 禁止「先撤净 STOP 再挂」；新挂失败 → **保留旧 STOP**（`place_failed_keep_old`）。 |
| 6 | **TP 成交必须价到 + 减仓证据** | 现价/best 触及该档 + 限价消失 + **头寸相对开仓基线确有减仓**才记账。自撤限价窗口禁止假成交。 |
| 7 | **现价已达 TP1（接管）→ 禁止重挂，只挂剩余档** | 开仓价+实时价两头对账：过 TP1 只挂 23；过 TP2 只挂 3；并评估是否启雷达。禁止无脑反复补挂 TP1。 |
| 8 | **重启接管禁止莫名平仓** | 不 force 档位裁减；scorch **只撤 TP、保留硬止损**；蚂蚁仓以外禁止核武全平。 |
| 9 | **钉钉同类不刷屏** | 标题去重（普通 ≥120s；系统告警/异常减仓 ≥600s）；攒批同标题折叠；事件锁防双线程双发。 |
| 10 | **三轨不抢份额** | TP123=`reduceOnly`；TV硬止损∪雷达=`closePosition` 单槽；雷达按档位激活线候命锁利。 |
...
#### 事故 C · SHORT/LONG 开仓成功但无 TP123、无 TV 硬止损

- **现象**：TV OPEN（含 tv_tp1/2/3）Webhook 成功，交易所有仓，盘口无线价止盈、无 closePosition。  
- **根因（叠加）**：  
  1. 硬止损「先撤净再挂」，挂失败 → 裸仓；  
  2. 触发价贴/穿市价被 **直接拒绝** 且不重挂；  
  3. 穿价 TP **整档跳过** → 可能 0 档 TP；  
  4. 开仓先撤 TP 后，现价贴近 TP1 → **误判「价到+限价消失=成交」** → 假记账 → 整档不挂；  
  5. 核武刹车 / 宽限期让哨兵只轻量补挂，全缺时迟迟不升级。  
- **修复**：≥ **v13.77** 先挂核实再撤孤儿、贴市推安全距；≥ **v13.78**  
  - TP 成交必须 **减仓证据**；自撤限价 30s 内禁止假成交；  
  - `_force_hang_open_defenses` 开仓/接管终检闭环；  
  - 穿价多轮加大间隙推离，禁止跳过全档；  
  - TP 全缺有仓 → 无视核武刹车与宽限，强制对齐。  
- **日志关键字**：`假成交` / `开仓强制挂防线` / `place_failed_keep_old` / `推高到安全` / `开仓终检`。

#### 事故 D · 重启/部署后 TP1 被反复补挂 → 仓位在 TP1 削光

- **现象**：VPS 重启接管后，以为「TP1 缺失」，不断重挂 TP1；现价已到/接近 TP1 时挂出即成交，仓位被吃光，到不了 TP23。  
- **根因**：老逻辑「无减仓证据就清空 tp_levels_consumed」→ 现价已过仍当漏挂；核武/补挂反复算 TP1。  
- **修复**：≥ **v13.79** `_apply_takeover_price_progress`：  
  1. 读开仓价 + 方向 + 实时价；  
  2. 现价已过 TP1 → **记账跳过，只挂 TP23**（过 TP2 则只挂 TP3）；  
  3. 达雷达激活比例或已过 TP1 → **启动雷达追随**（不要求「曾经交棒」）；  
  4. **禁止莫名平仓**；清除记账时保留「现价已过」档。  
- **日志关键字**：`开仓价/现价对账` / `现价已过TP` / `接管跳过补挂` / `重启/接管雷达激活`。
...
#### 事故 F · 执行层「开了仓却没挂防线」（铁律再次强调）

- **铁律（执行顺序，无例外）**：  
  1. **单独多/空 TV** → 先平（有仓则平）→ 再开 → **必须**挂 TV 头寸切片 TP123 + TV 硬止损；  
  2. **平+开同时到** → 同样先平后开 → 同样挂齐 TP123 + TV 硬止损；  
  3. **单独平 TV** → 平完 + 撤净挂单 → 干净等待下次 TV。  
- **份额关系**：TP123 = `reduceOnly` 限价；TV 硬止损与雷达 = **同一** `closePosition` STOP 槽（互不抢 TP 额度）。雷达开仓后待命，达激活线或 TP1 真成交后交棒替换硬止损。  
- **根因（2026-07-20 再现）**：无菌净场/`_full_reentry` 未回传 TV payload；开仓后 `tv_tps` 被冲掉；接管 `_takeover_price_skip` 残留污染开仓补挂。  
- **修复**：≥ **v13.81** `_snapshot_tv_open_defenses` → 净场后 `_bind_tv_open_defenses` → 开仓保护强制绑回；开仓路径强制 `_takeover_price_skip=False`。  
- **若再出现「有仓无 TP/无 STOP」**：查日志 `开仓前防线快照` / `绑定开仓防线` / `开仓强制挂防线`；确认 `health.version` ≥ v13.81。

### 三、雷达（开仓后 · 多空对称）注意点

| 项 | 正确行为 | 禁止 |
|----|----------|------|
| 开仓后 | 雷达**待命**；挂 TP123 + TV 硬止损；`POST_OPEN_RADAR_BLOCK_SEC≈180s` 内禁止近市雷达挂单 | 开仓立刻用雷达保本替换硬止损 |
| 启动条件 | 价触档位激活线（R1=50%…R4=80%）**或** TP1 **价到+限价消失** | 仅头寸变少就当 TP1 成交并交棒 |
...
# 开仓/裸仓/硬止损
grep -E '开仓终检|裸仓|假成交|开仓强制挂防线|place_failed_keep_old|推高到安全|推低到安全|强制TV硬止损|穿价' \
...
| v13.82 | **废除档位保证金%×25x；TV risk_pct/qty_ratio/leverage 唯一公式算仓** |
| v13.81 | **废除VPS宽硬止损；全面按TV tv_sl挂硬止损；UPDATE_SL同步盘口** |
...
**主题：删除多余 `position_supervisor.py`；生产唯一大脑 = `position_supervisor_binance.py`**

- 移除遗留 stub（旧保证金%/错误版本号），避免误跑旧逻辑
- `profit_taker` / `check_full_system` 改为指向 binance 军师或空壳
...
#### v13.84.0 · `tv-strategy-sync`

**主题：对齐终极策略 — 雷达50/60/70/80% · 废除VPS%宽止损 · 禁TP1补挂死循环 · 硬止损失败撤开仓**

- **雷达激活**对齐自查清单：R1=50% / R2=60% / R3=70% / R4=80%（entry→TP1）；WS mark@1s 实时候命
- **VPS_HARD_SL_PCT 清空**；`compute_vps_hard_sl` 恒0；实盘只挂 TV `tv_sl` 原值
- **TP123**：持仓期现价已达档 → 记账跳过，禁止补挂（杜绝 TP1 吃光仓）
- **硬止损失败** → `_emergency_flatten_naked_open` 市价撤开仓防裸奔
- 钉钉标题去重 300s；开仓确认含 TP/硬止损/雷达候命挂载状态
- 仓位仍唯一公式：TV `risk_pct`/`qty_ratio`/`leverage`

#### v13.83.0 · `tv-defense-iron`

- **唯一仓位公式**：`min(风险金额/止损距离, 权益×leverage/价) × qty_ratio`（无硬上限）
- VPS **直接使用** TV 下发的 `risk_pct` / `qty_ratio` / `leverage`，禁止旧「档位保证金% × 25x」回退
- 缺 `risk_pct`/`leverage` → 拒绝下单（禁止新老逻辑打架）
- 加仓同公式 × TV `qty_ratio`（不再 `base_qty × ratio`）
- 精度：`floor(qty×1000)/1000`；**已删除** `HARD_NOTIONAL_CAP`/`maxNotionalUSDT`

#### v13.81.0 · `tv-hard-sl-only`

**主题：废除 VPS% 宽硬止损；全面按 TV `tv_sl` 挂盘（多空）；禁止漏挂**

- `_tv_hard_sl_target`：实盘 STOP 唯一来源 = TV `tv_sl`（优先 `tv_sl_ref`，禁 VPS% 覆盖）
- `_looks_like_tv_tight_stop` 恒 False：废除「拒绝挂 TV 紧止损」
- `UPDATE_SL`：强制 `_sync_exchange_stop(force=True)` 改挂盘口；钉钉播报已同步
- 开仓/接管/重启：文案与硬闸一律「TV硬止损」；无 `tv_sl` → 告警 fail-closed

#### v13.80.0 · `open-tv-defense-bind`

**主题：TV 开仓强制绑定 TP123+硬止损；净场不得冲掉本笔防线**
...
**主题：重启接管按现价跳过已过 TP；只挂剩余档 + 按需启雷达；禁莫名平仓**

- `_apply_takeover_price_progress`：entry+mark 两头对账；过 TP1→只挂23；过TP2→只挂3
- 清记账/伪TP 清除：**保留现价已过档**（杜绝老逻辑清掉后再补挂 TP1）
- 雷达：达激活线或已过 TP1 → 接管即可激活（不要求曾交棒）
- 补挂/核武/重建：接管模式下现价已过 → 跳过不挂
- 禁止 force 档位裁减 / 禁止接管路径平仓

#### v13.78.0 · `open-defense-failclosed`

**主题：开仓/接管防线失败闭环；禁假成交导致 0 TP；禁 TP1 穿价死循环**

- TP 成交记账：价到 + 限价消失 + **减仓证据**；自撤 TP 30s 内禁止假成交
- 开仓/接管终检：`_force_hang_open_defenses`（清假成交 → 多轮推离穿价 → 挂 TP123 → 强制 VPS）
- 雷达守护：有仓 TP 全缺 → 无视宽限/冷却强制对齐 + 补硬止损
- 核武：TP 全缺有仓 → 无视 thrash 刹车
- 执行铁律不变：单独开 / 同秒开平 → 先平后开并挂齐；单独平 → 清场等待
...
- **TV 规则**：
  - 空仓仅 OPEN → 按档位直开 + TP123 + TV 硬止损 + 雷达待命
  - **雷达进行中**（已交棒/已激活）新 TV → **一律先平后开**（净场后再挂 TP123+宽止损+雷达待命）
  - 同K CLOSE+OPEN / 有仓换防 → 无菌先平后开
- **防死循环**：雷达追随禁止「先 scope=radar 撤再挂」；只走 `_sync_exchange_stop` 单槽合并（雷达∪TV硬止损）；同价已挂跳过；25s 步进门限
- **三轨共存**：TP=`reduceOnly`；雷达+TV硬止损=`closePosition` 单槽；互不抢份额
...
| **v13.82.0** | **废除档位保证金%×25x；TV risk_pct/qty_ratio/leverage 唯一公式（单笔硬上限已于 v13.85 删除）** |
| **v13.81.0** | **废除VPS宽硬止损；严格按TV tv_sl挂盘；UPDATE_SL同步盘口；禁漏挂/改回VPS%** |
...
| **v13.73.0** | **雷达适度追随：R1~R4 激活50/60/70/80%·步进35/30/25/20%·呼吸1.0/0.8/0.65/0.5ATR；删旧统一85%+阶段紧追；移动前TP价到对账；25s步进门限防撤挂死循环** |
