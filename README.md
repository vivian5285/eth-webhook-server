...
**当前版本：`v13.64.2-live-radar-recover-guard`**  
...
# 期望 version 含 v13.64.2-live-radar-recover-guard
...
```bash
python check_vps_logic.py          # 静态对账（无需 API Key）→ 期望 107 通过
...
├── 雷达待命 → 现价触激活线(R1/R2=70% · R3=75% · R4=80%)后交棒保本
├── 13x 总名义硬顶（双品种合计）
...
- **雷达价触激活线启动**：相对 entry→TP1；**交棒/重启只用现价**（禁止历史 best 误触保本）；废除「价+限价成交+减仓」三重强制门槛
- **头寸微漂 ≠ TP1 成交**：伪 TP 仍拦截记账；不挡雷达启动
- **开仓禁止裸仓**（v13.64）：TV 空字段必须本地补全 TP；`expected=0` 不得假齐；硬止损强制终检
- **重启禁止无故平仓**（v13.64.2）：未达激活线只挂 VPS 宽硬止损；曾交棒但现价回撤低于激活线 → 退回宽止损再待命
...
| 雷达价触激活线 | R1/R2=70% · R3=75% · R4=80%；TP2/TP3 锁利 | 🟡 P1 |
...
现价达档位激活线（R1/R2=70% · R3=75% · R4=80%；交棒不用历史 best）
...
| 场景 | 函数 | 说明 |
|------|------|------|
| 开仓 | `report_supervisor_open` | 核实持仓 + TP 对齐；含字段来源（TV/本地补全） |
| VPS 硬止损已挂 | `report_adverse_shield_armed` | 激活线前宽止损 |
| 雷达交棒 | `report_shield_disarmed` | **保本 STOP 核实后** |
| 雷达激活 | `report_radar_activated` | 首次保本，含进度 / 开仓 R |
...
|------|------------------|-------------|---------|------------------|
| R1 | **8%** | **25/35/40** | **2.78%** | **70%** |
| R2 | **14%** | **20/35/45** | **3.89%** | **70%** |
| R3 | **20%** | **18/32/50** | **5.56%** | **75%** |
| R4 | **26%** | **5/20/75** | **8.33%** | **80%** |
...
# 期望: v13.64.2-live-radar-recover-guard
...
# version 含 v13.64.2 · tv_strategy 含 v6.9.108
...
| 开仓后无 TP、无硬止损 | 升级 ≥ v13.64.2；查「开仓 TP123 补全」「开仓终检裸仓」「开仓滞后核实」；人工先挂 closePosition |
| 只有 TP23 无 TP1 | 查 `伪TP` / `trusted_initial` / 重启 `_ensure_full_defense_stack` |
| 微盈就全平、无雷达钉钉 | 查「交棒延迟」/ SHORT 抬过成本；确认开仓冷却 180s；重启是否误用历史 best |
...
*GEMINI Quant · 双轨智慧雷达 · v13.64.2-live-radar-recover-guard*
