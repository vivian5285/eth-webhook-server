...
| 4 | **开仓必须有 TP123 + VPS 宽硬止损** | 缺任一视为裸仓；终检 `_force_hang_open_defenses` 闭环补挂；钉钉告警。雷达开仓后仅待命，不替代硬止损。 |
...
#### 事故 C · SHORT/LONG 开仓成功但无 TP123、无 VPS 宽硬止损
...
- **铁律（执行顺序，无例外）**：  
  1. **单独多/空 TV** → 先平（有仓则平）→ 再开 → **必须**挂 TV 头寸切片 TP123 + VPS 宽硬止损；  
...
# 开仓/裸仓/硬止损
grep -E '开仓终检|裸仓|假成交|开仓强制挂防线|place_failed_keep_old|推高到安全|推低到安全|强制VPS宽硬止损|穿价' \
