...
**当前版本：`v13.75.0-force-close-then-open`**  
...
# 期望 version 含 v13.75.0-force-close-then-open
...
- **同秒开平铁律**（v13.75）：同时收到开仓+平仓 → 短停聚合 → **先平后开**（终态必须有仓）；即使 OPEN.seq=1、CLOSE.seq=2 也强制纠正
...
- **时序先平后开**（v13.68/v13.70/v13.75）：同K/同秒 CLOSE+OPEN → 强制先平后开；**空仓仅 OPEN → 按档位直接开满，禁止画蛇添足秒平**
...
空仓仅 OPEN → **按档位直开** + TP123 + VPS宽硬止损 + 雷达待命（不做先平后开）。  
空仓短时重复同向信号 → 忽略 + 钉钉（无 `bar_index`/`seq` 时指纹去重约 45s）。  
同K/同秒同时开+平（含 OPEN.seq=1 + CLOSE.seq=2）→ **短停聚合后强制先平后开**（终态必须有仓）。
### 动态加仓
...
# 期望: v13.75.0-force-close-then-open
...
# version 含 v13.75.0 · tv_strategy 含 v6.9.108
...
| 同秒开平后空仓（先开后秒平） | 升级 ≥ **v13.75**；查「强制先平后开」「同秒开平」；钉钉先平后开链 |
| 开完立刻秒平大半 | 升级 ≥ v13.71（禁穿价 TP）；查「穿价」「无菌」「档位裁减」 |
| 头寸少了却报 TP12 成交 | 升级 ≥ v13.72；必须价到才记账；查「拒认」「异常减仓·非TP」 |
...
*GEMINI Quant · 双轨智慧雷达 · v13.75.0-force-close-then-open*
