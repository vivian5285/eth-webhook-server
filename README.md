...
**雷达（规格 §5.1）**：弱/中/强档步进；**ADX 启动 70%~90%**（≤17→70% · ≥35→90% · 中间线性）× **1.35×initial_atr**；首次/重入同一公式；激活臂 **entry±0.5×TV.atr**；达线前盘口**仅硬止损**；**独立于 TP1 是否成交**  
...
> **雷达（规格 §5.1 · v16.7.0）**：启动价=`entry ± ADX比例×1.35×ATR`（70%~90%连续插值）；首次/重入同一公式；TP1 成交不挡启动。  
> **v16.7.0**：正式替换中点/TP2 绝对价闸 → ADX 驱动启动比例；第二层 trail 插值不动。  
...
- **启动（第一层 · ADX 比例 · v16.7.0）**：
  - `ratio = lerp(70%, 90%, clamp((ADX−17)/(35−17), 0, 1))`
...
- **第二层 trail**：实时 ATR/initial_atr 连续插值（ETH 1.2~2.5 · XAU 0.5~1.2 · floor/ceiling 0.6/2.2）— **本次不改**
- **分区呼吸**：TP1–TP2 宽松 → TP2–TP3 收紧 → TP3+ 动态追踪（`min_mult`~`max_mult`）
- **重入**：仅雷达扫出且微赚区间；最多 **1** 次；窗口 ETH **2** 根 90m · XAU **3** 根 45m；价须优于上次开仓；成功后雷达放宽一档；启动仍用冻结 ADX 比例；**TP1 已成交 / 非强趋势禁重入**
...
再入微赚区：ETH ±0.5×ATR · XAU ±0.3×ATR。配置源：`config/reentry_tiers.json`（schema `v3.2-adx-act-70-90`）。  
...
**一句话**：硬止损是底线（呼吸垫统一 1.15）。雷达启动按 ADX 在 70%~90%×1.35ATR 间连续插值——弱趋势更早锁保本，强趋势给更多奔跑空间。重入最多一次，有窗口，价必须更优。
