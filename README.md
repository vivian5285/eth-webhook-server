...
**当前版本：`v13.84.0-tv-strategy-sync`**  
...
# 期望 version 含 v13.84.0-tv-strategy-sync
...
| 开单 sizing | TV `risk_pct`/`leverage`/`qty_ratio` → min(理论,杠杆限,50000/价)×ratio | 🔴 P0 |
...
| 类型 | sizing 规则 |
|------|-------------|
| **OPEN** | `min(理论, 杠杆限制, 硬上限) × qty_ratio`；直接用 TV `risk_pct`/`leverage`/`qty_ratio` |
...
# version 含 v13.84.0 · tv_strategy 含 v6.9.108
...
> 当前修复版本基准：**`v13.84.0-tv-strategy-sync`**（先平后开+TP123+TV硬止损+雷达待命铁律；含 v13.75～v13.82）。
...
- **唯一仓位公式**：`min(风险金额/止损距离, 权益×leverage/价, 50000/价) × qty_ratio`
...
- 精度：`floor(qty×1000)/1000`；硬上限 `HARD_NOTIONAL_CAP=50000`
...
| **v13.82.0** | **废除档位保证金%×25x；TV risk_pct/qty_ratio/leverage 唯一公式；硬上限50000U** |
...
*GEMINI Quant · 双轨智慧雷达 · v13.84.0-tv-strategy-sync*
