...
**ADX 三档雷达（规格 §5.1）**：弱<20 / 中20–30 / 强>30；**绝对价锚定** — 首次 **(TP1+TP2)/2** · 重入 **TP2**（共用同一套 TP 绝对价）；激活臂 **entry±0.5ATR**；达线前盘口**仅硬止损**  
...
> **硬止损（唯一公式 · v16.2.0）**：`|TV价 − TV.stop_loss| × 1.15`，挂在**成交价**外侧。缺/异常 `stop_loss` → **拒开**。巡检/接管禁止用 0.5×ATR 顶替。  
> **TP（v15.9.0）**：10%/20%/70% 常挂 TP1+TP2+TP3；TP3 与雷达互斥（谁先成交撤另一腿）。  
> **雷达（规格 §5.1 · v16.3.7）**：首次激活价=`(TP1+TP2)/2`，重入激活价=`TP2`；**禁止**误用首次中点去激活重入单（过中点未到 TP2 仍须休眠）。  
...
> **v16.3.7**：雷达绝对价门（中点/TP2）+ 钉钉/TG「启动门槛」文案；假裸仓告警抑制；休眠禁双 STOP。  
> **v16.3.3–v16.3.6**：REST 滞后硬止损、休眠单硬、禁 force 挂雷达、限流恢复路径。  
> **v16.3.1–v16.3.2**：双通道通知 +【币安单系统】品牌。  
> **v16.3.0**：规格切片/重入闸门/防叠限价。
