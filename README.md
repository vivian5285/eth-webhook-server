...
**当前版本：`v13.73.0-radar-moderate-follow`**  
...
# 期望 version 含 v13.73.0-radar-moderate-follow
...
├── 雷达待命 → 档位激活线(R1=85%…R4=70%)或TP1成交后适度追随保本
...
- **雷达适度追随**（v13.73）：按档启动 R1=85%/R2=80%/R3=75%/R4=70%；步进 35/30/25/20%；呼吸 1.0/0.8/0.65/0.5 ATR；删除旧统一85%+阶段紧追；禁撤挂死循环
...
- **时序先平后开**（v13.68/v13.70）：仅当同K同时有 CLOSE+OPEN（或实盘有仓需换防）才先平后开；**空仓仅 OPEN → 按档位直接开满，禁止画蛇添足秒平**
...
现价达档位激活线（R1=85%…R4=70%）或 TP1 已真实成交
...
# 期望: v13.73.0-radar-moderate-follow
...
# version 含 v13.73.0 · tv_strategy 含 v6.9.108
