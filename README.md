...
4. **止损单全局唯一写入方 = 呼吸止损引擎**  
   ...
   保留兜底：`HARD_SL_FAIL_ABORT`（改单失败重试告警）、`CLOSE_THEN_OPEN_FAIL_ABORT`（先平后开净场失败中止）、`FORCE_ALIGN`（重启方向不一致强制对齐）。  
...
TradingView v6.5.6 Alert (secret=528586)
        │
        ▼
   app.py  /webhook   ← 解析 · secret · 品种路由 · 异步线程
        │
        ▼
...
2. 仍失败 → **放弃本笔开仓**（严禁在平仓结果不明时继续开新仓）
3. 高优先级钉钉「清仓失败·需人工介入」
4. 将该 symbol 置 `trading_paused=CLOSE_THEN_OPEN_FAIL_ABORT`（空仓也不自动解除）
5. 人工核对币安持仓/挂单清净后：`POST /admin/resume/ETHUSDT` 恢复

### 6.8 ATR 应急降级（临时·高调·需人工恢复）

**定位：** 极端情况下临时用 TV 隐含 ATR，**不是**常态备选，禁止静默切换。

触发（任一）：
1. VPS 拉不到足够 K 线 / ATR 空或 ≤0
2. VPS ATR < 近50根中位数 × 30%
3. VPS vs TV隐含（`|price−stop_loss|/1.0`）偏差 ≥20%，且**连续 3 次**开仓信号

降级时：
- `initialAtr = TV隐含ATR`；`initialStop = entry ± 1.5 × TV隐含ATR`
- 呼吸止损其余倍数（0.75 / 0.4 / ADX 追踪等）**不变**
- 本笔仍开仓；钉钉 `ATR_DEGRADE_MANUAL_RESUME`；标签 `atr_source=tv_implied_degrade`
- 随后暂停该 symbol 自动开仓，复验后 `POST /admin/resume/{SYMBOL}`
...
> **注意（2026-07-22 误告警）：** 钉钉「ATR核对差异 Δ29%~33%」曾把 `|price−stop_loss|/1.5` 当成「TV ATR」。  
> TV 硬止损常见约 **1.0×ATR**，VPS `initialStop` 才是 **1.5×ATR**，误用 1.5 反推会系统性报出约 33% 假偏差。  
> 现已改为：优先比对 webhook `atr`；否则按 `TV_HARD_SL_ATR_MULT=1.0` 反推。该核对**仅日志/提示，不拦截开仓**。
...
| 先平后开净场失败 | 3 次重试(1s/3s/6s) → CLOSE_THEN_OPEN_FAIL_ABORT（暂停开仓，需 `/admin/resume`） |
