# app.py（最终升级版 - 带详细账户快照 + 实际TP价格）
from flask import Flask, request, jsonify
import time
import traceback
import logging
from binance_client import BinanceClient
from position_manager import PositionManager
from tp_manager import calculate_tp_prices, get_actual_tp_prices
from dingtalk import send_dingtalk
from config import Config

logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s')

app = Flask(__name__)
client = BinanceClient()
position_manager = PositionManager()

@app.route('/webhook', methods=['POST'])
def webhook():
    data = request.get_json()
    if not data:
        return jsonify({"status": "error"}), 400

    try:
        process_webhook(data)
        return jsonify({"status": "success"}), 200
    except Exception as e:
        logging.error(f"[CRITICAL] {traceback.format_exc()}")
        return jsonify({"status": "success"}), 200   # 避免 TradingView 一直重试


def process_webhook(data: dict):
    signal = data.get("signal")
    symbol = data.get("symbol", "ETHUSDT")
    atr = data.get("atr")
    reason = data.get("reason", "")

    logging.info(f"[收到信号] {signal} | {symbol}")

    if signal in ["OPEN_LONG", "OPEN_SHORT"]:
        try:
            # 检查并平掉反向持仓
            current_pos = client.get_current_position(symbol)
            if current_pos and float(current_pos.get("positionAmt", 0)) != 0:
                logging.info("[风控] 检测到持仓，先全平")
                client.close_all_positions(symbol)
                time.sleep(1.5)

            qty = client.calculate_position_size(atr)
            if qty <= 0:
                send_dingtalk("风控拦截", f"计算仓位为 {qty}，拒绝开仓", is_warning=True)
                return

            if signal == "OPEN_LONG":
                order = client.open_long(symbol, qty)
                if order:
                    entry_price = float(order.get("avgPrice", 0))
                    tp_prices = get_actual_tp_prices(entry_price, atr, "long")  # ← 实际价格

                    position_manager.save_position(symbol, entry_price, atr, tp_prices, "long")

                    report = client.get_detailed_report()
                    msg = (
                        f"**下单数量**: {qty}\n"
                        f"**入场价**: {entry_price}\n"
                        f"**TP1**: {tp_prices['tp1']} | **TP2**: {tp_prices['tp2']} | **TP3**: {tp_prices['tp3']}\n\n"
                        f"**账户快照**\n"
                        f"总权益: {report['total_equity']} USDT\n"
                        f"钱包余额: {report['wallet_balance']} USDT\n"
                        f"可用保证金: {report['available_margin']} USDT\n"
                        f"当前持仓: {report['position']}\n"
                        f"浮盈: {report['unrealized_pnl']} USDT"
                    )
                    send_dingtalk("LONG 开仓成功", msg)

            elif signal == "OPEN_SHORT":
                order = client.open_short(symbol, qty)
                if order:
                    entry_price = float(order.get("avgPrice", 0))
                    tp_prices = get_actual_tp_prices(entry_price, atr, "short")

                    position_manager.save_position(symbol, entry_price, atr, tp_prices, "short")

                    report = client.get_detailed_report()
                    msg = (
                        f"**下单数量**: {qty}\n"
                        f"**入场价**: {entry_price}\n"
                        f"**TP1**: {tp_prices['tp1']} | **TP2**: {tp_prices['tp2']} | **TP3**: {tp_prices['tp3']}\n\n"
                        f"**账户快照**\n"
                        f"总权益: {report['total_equity']} USDT\n"
                        f"钱包余额: {report['wallet_balance']} USDT\n"
                        f"可用保证金: {report['available_margin']} USDT\n"
                        f"当前持仓: {report['position']}\n"
                        f"浮盈: {report['unrealized_pnl']} USDT"
                    )
                    send_dingtalk("SHORT 开仓成功", msg)

        except Exception as e:
            logging.error(f"[开仓异常] {e}")
            send_dingtalk("开仓异常", str(e), is_warning=True)

    elif signal == "CLOSE_ALL":
        try:
            client.close_all_positions(symbol)
            position_manager.clear_position(symbol)
            report = client.get_detailed_report()
            send_dingtalk("保护性全平", f"原因: {reason}\n当前已空仓\n总权益: {report['total_equity']} USDT")
        except Exception as e:
            logging.error(f"[平仓异常] {e}")
            send_dingtalk("平仓异常", str(e), is_warning=True)
