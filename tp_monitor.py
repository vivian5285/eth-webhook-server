...
class TPMonitor:
    ...
    def start(self):
        ...
        logging.info(f"[TP监控] WebSocket实时监控已启动 | 品种: {self.symbol}")
    ...
    def _on_price_update(self, msg):
        ...
        try:
            ...
            logging.error(f"[WebSocket价格更新异常] {e}")
            ...
            try:
                ...
                if side == "long":
                    if "tp1" not in hit and price >= tp.get("tp1", 0):
                        ...
                        self._execute_tp("tp3", price, pos, 1.0)   # TP3 全平剩余
                else:
