# tp_monitor.py（最终版 - 已接入 supervisor 汇报）
import time
import logging
...
class TPMonitor:
    ...
    def start(self):
        if self.running:
            ...
            return
        self.running = True
        logging.info("[TPMonitor] TP监控已启动（最终版 - 通过 supervisor 汇报）")
        while self.running:
            try:
