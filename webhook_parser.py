...
# 2026-08-17：用户决定进一步全面调低，弱/中/强三档从0.7/0.8/1.0改成
# 0.3/0.5/0.8，全部品种统一生效（SYMBOL_TIER_NOTIONAL_MULT仍是空表，
...
# 2026-08-18（当天再调）：用户要求"细水长流"，弱/中/强三档从0.2/0.3/0.5
# 继续下调到0.1/0.2/0.3，全部品种统一生效。
# 2026-08-18（当天第三次再调）：继续下调，弱/中/强三档从0.1/0.2/0.3
# 降到0.05/0.1/0.2，全部品种统一生效。
# 2026-08-18（当天第四次，用户确认不再改）：回调到0.1/0.2/0.3，"细水
# 长流"，全部品种统一生效。用户明确表示这次是最终值。
# 2026-08-18（当天第五次）：从0.1/0.2/0.3再调到0.2/0.35/0.5，全部品种统一生效。
# 2026-08-19：用户在现有基础上整体再降30%，弱/中/强三档从0.2/0.35/0.5
# 降到0.14/0.245/0.35，全部品种统一生效。
# 2026-09-04：品种数量已经涨到17个(比8月18日那次调整时的12/13个更多)，
# 用户反馈"每天都有不同的币和美股磨损，一个月亏损率特别大"，要求在现有
# 基础上整体再砍70%(降到约30%权重)——弱/中/强三档从0.14/0.245/0.35
# 降到0.042/0.0735/0.105，全部品种统一生效。同一批调查过30天真实已实现
# 盈亏(futures_income_history)：B账户-139.31、C账户-44.69都是分散在很多
# 品种上的小额磨损，符合用户诊断；E账户-1970.15表面看起来夸张得多，但
# 挖出真实成交明细后发现里面-1814.99几乎全部来自单笔历史事件(2026-08-21
# 01:48:17，Hedge Mode下同时扛着SHORT 6.959+LONG 8.187 ETH两个方向的仓位
# 同时平仓，-4866.07+3051.87=-1814.2，那次Hedge Mode配置问题已经在
# 2026-08-22修过，30天窗口内再没有出现过同类型的大额单笔事件)——排除这
# 一笔孤立历史事件后，E账户真实的"磨损型"月度亏损约-156，跟B/C同一个量级，
# 三个账户的实际情况是一致的"多品种分散小额磨损"，本次下调是对症的。
TIER_NOTIONAL_MULT = {0: 0.042, 1: 0.0735, 2: 0.105}

# 2026-09-04（同一批，宝贝要求"你的最优解"）：全局70%已经把好品种也一起
# 砍了一遍，不该再一刀切——查了30天逐笔已实现盈亏(futures_income_history,
# 按品种拆:笔数/胜率/均盈/均亏/期望值每笔)，B+C+E合并、剔除E账户8/21那笔
# 已修复的Hedge Mode孤立历史事件后，真正在磨损本金的集中在5个品种，其余
# 12个品种要么小额浮动要么净赚(ANTHROPIC+19.75/GS+14.20/XAU+11.51/MU+7.34
# 等)，跟着一起降是误伤，只针对这5个品种在全局70%基础上再单独收紧：
#   XMRUSDT/SKHYNIXUSDT：期望值每笔-0.571/-0.321，161/154笔样本量足够大，
#     确认是持续磨损不是运气——再收到全局值的30%（即弱/中/强
#     0.0126/0.02205/0.0315）。
#   BNBUSDT/ZECUSDT：期望值每笔没有前两个那么负(-0.207/-0.130)，但笔数
#     全品种最高(348笔/409笔，30天里日均好几笔)，磨损主要靠频率堆出来——
#     收到全局值的40%（弱/中/强0.0168/0.0294/0.042）。
#   XPDUSDT：66笔里只赢6笔，胜率9.1%——明显不是"趋势型策略正常等趋势"
#     能解释的，比其余4个品种反常得多，值得怀疑这个品种当前的TV信号跟
#     策略本身不匹配，不只是磨损问题。宝贝的意思是"宁可少下单"，这里没有
#     用运维层面的trading_paused硬暂停(那是运行时状态，需要额外的接口
#     调用/人工resume流程)，而是直接把权重压到全局值的10%（弱/中/强
#     0.0042/0.00735/0.0105，约等于原始0.14/0.245/0.35基线的0.7%）——
#     跟既有的_normalize_tp_qty_map(见position_supervisor_binance.py)
#     配合，新开仓大概率小到连TP1/TP2限价单都摆不出来，直接落到"只挂硬
#     止损+雷达"的既有安全通路，实质上等同于新仓位近乎不产生，同时不需要
#     额外的运维动作，胜率后续回升了随时把这三个数字调回正常水平即可。
SYMBOL_TIER_NOTIONAL_MULT = {
    "XMRUSDT": {0: 0.0126, 1: 0.02205, 2: 0.0315},
    "SKHYNIXUSDT": {0: 0.0126, 1: 0.02205, 2: 0.0315},
    "BNBUSDT": {0: 0.0168, 1: 0.0294, 2: 0.042},
    "ZECUSDT": {0: 0.0168, 1: 0.0294, 2: 0.042},
    "XPDUSDT": {0: 0.0042, 1: 0.00735, 2: 0.0105},
}
...
# 2026-08-27：新增TSLA，活跃品种共17个，16→17同理由。
# 2026-08-27：同日再新增META，活跃品种共18个，17→18同理由。
# 注意：以后再调TIER_NOTIONAL_MULT/SYMBOL_TIER_NOTIONAL_MULT里任何品种的
# 强档倍数、或停用/启用某品种的TV、或再加新品种，这个值要跟着重算，否则
# 要么裁过头要么裁不住。
MAX_TOTAL_NOTIONAL_MULT = 18.0
