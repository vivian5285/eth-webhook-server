...
# 2026-09-05：曾经把杠杆假设从5降到3(仓位金额不变，杠杆下降，换算下降)。
# 2026-09-06：宝贝反馈"改回5倍杠杆吧，太低杠杆，感觉失去开单意义，时间
# 也是金钱"——改回来。仓位公式与交易所真实杠杆本来就彻底解耦(见下面
...
# 2026-09-04（当天再调）：部署后宝贝担心"会不会砍得太凶"，改成在原始
# 0.14/0.245/0.35基础上降50%（不是70%）——弱/中/强三档从0.042/0.0735/
# 0.105上调到0.07/0.1225/0.175，全部品种统一生效。
TIER_NOTIONAL_MULT = {0: 0.07, 1: 0.1225, 2: 0.175}

# 2026-09-04（同一批，宝贝要求"你的最优解"后短暂加过、当场撤回）：查了
# 30天逐笔已实现盈亏(futures_income_history，按品种拆笔数/胜率/均盈/均亏/
# 期望值每笔)，B+C+E合并、剔除E账户8/21那笔已修复的Hedge Mode孤立历史
# 事件后，发现XMRUSDT/SKHYNIXUSDT(期望值每笔-0.571/-0.321)/BNBUSDT/
# ZECUSDT(笔数全品种最高348/409笔)/XPDUSDT(胜率仅9.1%，66笔只赢6笔，明
# 显反常)这5个品种磨损最重，一度在这里给它们单独加过比全局70%更狠的
# per-symbol覆盖(30%/40%/10%)。宝贝当场反馈：XMR/XPD这两个宝贝自己也
# 独立发现了异样，但**今天同一批已经做了好几处TV策略/雷达耐心模式的
# 优化**(gate形态重建、耐心模式全品种恢复等)，这些优化本身可能已经在
# 改善这几个品种的表现——不想再叠加per-symbol的特殊照顾，宁可先统一用
# 全局70%这一道观察一阵子，看今天这批优化自己的效果，而不是同时改两个
# 变量搞不清哪个起作用。所以撤回了这份per-symbol覆盖，SYMBOL_TIER_
# NOTIONAL_MULT保持空——全部17个品种统一吃刚才那条全局70%(0.042/0.0735/
# 0.105)。这5个品种的诊断数据本身没有过期，只是暂缓行动；如果观察一段
# 时间后这5个品种(尤其XPD的9.1%胜率)还是没有改善，这份per-symbol覆盖的
# 具体数字可以直接原样重新启用。
