...
## 2026-08-04 · 雷达止损贴市误判修复（v16.20）

### 现象
XAU实盘雷达止损已挂在4040.50，但系统日志持续报错"雷达止损挂单未核实"并触发 HARD_SL_FAIL_ABORT。

### 根因
`_place_vps_hard_sl_order` 检测到"止损价贴市"时直接返回 None，但在贴市检测之前止损单可能已经挂在交易所了（上一轮挂单成功，只是因为价格已经贴市所以再次尝试挂单时被拒）。系统误判已挂单为失败。

### 修复
- `_place_vps_hard_sl_order` 贴市检测时先检查是否已有止损单存在
- 若已有同价止损单，返回成功（止损价）而非 None
- 修复影响：ETH / XAU / BNB 三品种
...
### 复查点
- [ ] VPS 重启后 TP1 是否正确补挂
- [ ] 日志应出现「接管强检：TP1标记消费但头寸未减+限价消失 → 判定为漏挂」
- [ ] ETH / XAU / BNB 三品种均正常

---

## 2026-07-28 · 规格 v1.0激活比例写反修复（v16.8.1 / 深币 v13.92.2）

### 现象
v16.8.0 把 ADX 启动比例写反：弱趋势 85%（太晚）、强趋势 70%（太早），与「弱早强晚」设计相反。

### 根因
离散三档常量与文档意图相反；弱趋势微利易被毛刺吞没，强趋势深度回踩被过早保本扫出。

### 修复
- `activation_ratios` → 弱 **68%** / 中 **78%** / 强 **88%**（落在 65–70 / 75–80 / 85–90）
- schema `v4.1-marathon-act-fix`；`normalize` 迁移旧冻结 0.85/0.70
- 币安 `v16.8.1-marathon-act-fix` + 深币 `v13.92.2-marathon-act-fix` 三端对齐

### 复查点
- [ ] `/health` 币安=`v16.8.1-marathon-act-fix`；深币 5004/5005=`v13.92.2-marathon-act-fix`
- [ ] 单测：弱触线价 < 强触线价（多单）
- [ ] 休眠仓冻结比例已按 ADX 重算

---

## 2026-07-28 · 深币主脑对齐规格 v1.0雷达（v13.92.0-marathon-radar）

### 现象
深币激活仍用 entry±0.4×ATR「保本底线」，交棒撤硬止损；与币安 v16.8.0 规格 v1.0不一致。
...
- [ ] 实盘激活日志「规格 v1.0雷达已激活·保本起步」
- [ ] 激活后硬止损单仍在

---

## 2026-07-28 · 规格 v1.0雷达保本起步（v16.8.0-marathon-radar）

### 现象
雷达激活后止损被推到 TP1 强制底线（约 entry+0.5×ATR）；激活位置本身就在 TP1 附近，止损与市价贴在一起，正常回撤即扫出，放弃 TP2/TP3 大部分利润。

### 根因
激活臂用 entry±0.5ATR + TP1/TP2 利润地板抬升，与「利润奔跑」目标冲突。

### 修复
- 激活瞬间改为**保本起步**：entry ± tick ± fee_cover（约 0.08%）
- **取消** TP1/TP2 强制底线；`_ratchet_radar_profit_floor` 废除为 no-op
- ADX 启动改为离散三档：弱85% / 中80% / 强70%（强趋势更早守护）
- TP 成交只缩量不改价；阶梯从保本位推进；浮盈≥3×ATR 切入雷达动态追踪阶段
- 硬止损始终独立并存

### 复查点
- [ ] `/health`=`v16.8.0-marathon-radar`
- [x] 单测 `test_radar_reentry` + `test_breath_radar_upgrade`
- [ ] 实盘激活日志含「保本起步」；TP1 成交后 SL 价不跳变
- [ ] 20U 多空演练（双币种）
