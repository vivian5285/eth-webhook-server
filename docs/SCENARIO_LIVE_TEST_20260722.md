...
| 6 | SHORT 完整周期 | ✅ **真实验证通过** |
| 1 | 雷达阶梯推进阶段阶梯止损真实移动 | ⏳ **未触发**（提前收尾；max fav≈4.63 &lt; 10.37） |
| 2 | TP1/TP2 真实成交 + 止损 qty 收缩 | ⏳ **未触发**（价未到测试 TP） |
| 8 | 跨越 90m K 线 ADX 更新 | ⏳ **窗口不足**（ADX 采样恒 28.74） |
| 9 | TP 超时门控真实分支 | ⏳ **未触发** |
| 3 | 进入雷达动态追踪阶段（3.0×ATR） | ⏳ **留待生产自然触发**（需浮盈≈41.5） |
...
| 12:53:32 | 雷达止损 | **1941.43** = entry + 1.5×ATR（公式一致） |
...
| 最大浮盈 | fav≈**4.63**（需 ≥10.37 才阶梯；需 ≥41.47 才雷达动态追踪阶段） |
...
留待生产自然触发（雷达动态追踪阶段 / 真实止损 / TP3 追踪全平）。
