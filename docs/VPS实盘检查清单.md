...
| 2.7 | Σ名义 ≤ 总权益 × 9 | ✅ | `check_total_notional_cap()` |
...
```
价格达 TP1 区（主）
  AND 账本 TP1 已消费 + 盘口无 TP1 限价（辅）
  AND 减仓量匹配 TP1 切片且 > 噪声阈值（参考）
→ _radar_armed_after_tp1 = True → 交棒保本 STOP
```
...
1. ETH 1800 R3 → 保证金 150U，名义 3750U，qty 2.08
2. 硬止损 1700（5.56%）
...
1. 1000U 本金，ETH R4 4500U + XAU R4 4500U = 9000U ✅
2. 若已有 9500U 名义 → 拒绝新开仓
