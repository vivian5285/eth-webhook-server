...
STRATEGY_DESCRIPTIONS: Dict[str, str] = {
    ...
    "mtf_ema_macd_cci": (
        ...
        "主触发全在 4h：收盘突破近10根高/低点 + 当根放量(≥1.3×量能加权"
        "均量) + 收盘价落在振幅强侧(强阳/强阴线，不是假突破)。EMA7/30、"
        "MACD柱、CCI 降级成轻确认(不逆4h EMA30、MACD柱不在反方向、CCI "
        "动量同向)。日线 EMA7/30 **只定仓位档位**(同向=tier2、否则tier1)，"
        "绝不否决交易。离场裸K优先：收盘跌破入场那根4h K线低点 / 放量强"
        "反向K线 / 跌破近5根结构低点 / 4h EMA 反向交叉。快指标选 CCI——"
        "本擂台 RSI/随机K都被用过，CCI 没人用、无界对短冲量敏感，这里只"
        "当动量方向的轻确认。跟 ema_cross_7_30/macd_histogram(单周期纯"
        "指标)、mtf_ema_pullback(等回踩)、turtle_breakout/breakout_retest"
