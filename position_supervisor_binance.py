...
class PositionSupervisorBinance(PipelineBridgeMixin, RadarReentryMixin):
    ...
    def _apply_breath_stop_tick(self, curr_px=0.0):
        ...
        # 它是最快速反应市场趋势的、最敏捷的一个"，优先让双均线先判断。
        # 真实放量确认破位时直接_close_all()市价清仓(内部自己处理，不等
        # ladder继续跑)；没放量确认时收紧到"至少不低于保本线"(内部顺带
        # 算了一次跟反转锁盈同一个保本公式，取更紧的那个)，即使这次没有
        # 放量确认，双均线的敏捷判断本身也不会比反转锁盈更保守。A系统
        ...
        # 2026-09-01新增：TV已平仓滞涨刹车——见radar_reentry_mixin.py
        # "2026-09-01新增"顶部注释。同样只朝有利方向棘轮，写在回吐刹车
        # 之后，四个棘轮谁锁得更紧生效谁的，调用顺序不影响最终结果。
        try:
            self.current_sl = self._maybe_tighten_on_tv_exit_stall(px, float(self.current_sl or 0))
            self.tv_sl = float(self.current_sl)
        except Exception as e:
            logger.debug(f"[{self.symbol}] TV已平仓滞涨刹车复评跳过: {e}")
        was_phase = phase
